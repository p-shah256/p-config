...
-- cp -r nvim-treesitter/runtime/queries ~/.local/share/nvim-learn/site/
...
local function install_ts_parser(name, url, force)
	...
	local cmd = string.format(
		"%s -o %s -I%s %s -shared -fPIC -O2",
		cc, so_path, src, files
	)
...
local function highlight(bufnr)
	...
	local function walk(n) -- if same type highlight else keep walking to other children
		if cursor_n:type() == n:type() and vim.treesitter.get_node_text(n, bufnr) == cursor_text then
			...
			vim.api.nvim_buf_set_extmark(bufnr, ns_id, sline, scol,
...
--
-- TODO: add a quickfix list of all the files changed in curr commit and unstaged
-- sl show or sl diff
-- and then navigate with quickfix list to see changed files
...
vim.api.nvim_create_autocmd({ 'BufReadPost', 'BufWritePost' }, {
	callback = function(ev)
		...
		vim.system(VCS_CMD(file), {}, vim.schedule_wrap(function(r)
			...
			for _, h in ipairs(vim.text.diff(r.stdout, curr, { result_type = 'indices' })) do
				local sign = h[2] == 0 and { '+', 'DiffAdd' } or h[4] == 0 and { '-', 'DiffDelete' } or
				{ '~', 'DiffChange' }
				local from = h[4] == 0 and h[3] or h[3]
				local to = h[4] == 0 and h[3] or h[3] + h[4] - 1
				for lnum = from, to do
					vim.api.nvim_buf_set_extmark(ev.buf, diff_ns, lnum - 1, 0,
...
vim.keymap.set('n', '<leader>dv', function()
	if diff_buf and vim.api.nvim_buf_is_valid(diff_buf) then
		...
		for _, w in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
			if vim.api.nvim_win_get_buf(w) == diff_buf then vim.api.nvim_win_close(w, true) end
		end
	...
	local r = vim.system(VCS_CMD(file)):wait()
...
vim.keymap.set('n', '[c', function()
	...
	if hunks[i][3] + hunks[i][4] - 1 < vim.fn.line('.') then
			vim.fn.cursor(hunks[i][3] + hunks[i][4] - 1, 1); return
		end
	end
...
vim.keymap.set('n', ']c', function()
	...
	for i = 1, #hunks do
		if hunks[i][3] > vim.fn.line('.') then
			vim.fn.cursor(hunks[i][3], 1); return
		end
	end
