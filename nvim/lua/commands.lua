...
--    tabstop      : Width of a raw  \t  character. (used for viewing)
--    shiftwidth   : The number of spaces used for automatic indentation. (\t if expand tab is set)
--    softtabstop  : Tab width during editing/backspacing. (used for editing)
--    expandtab    : if on, then use spaces instead of tabs.
--    filetype     : Language/type of current file.
--    indentexpr   : Custom code for auto-indent.
...
local function update_leadmultispace()
        local sw = 0
        if indent_options_are_default() then
		...
		-- stylua: ignore
                if not min_indent then sw = 8 -- round it down to even
                elseif min_indent <= 2 then sw = 2
                elseif min_indent <= 4 then sw = 4
                else sw = 8 end
        else -- if not default, meaning someone has already set shiftwidth
        ...
        -- Set indentation rules for the current buffer
