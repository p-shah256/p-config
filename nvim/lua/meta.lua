...
local function join(dir, path)
	return path:sub(1, 1) == "/" and path or dir .. "/" .. path
end

local function current_sl_root()
	return vim.fn.systemlist("sl root 2>/dev/null")[1] or vim.fn.getcwd()
end
...
vim.api.nvim_create_user_command("Qf", function(opts)
	...
	if cmd:match("^myles%s") then
		...
		for _, line in ipairs(lines) do
			if line ~= "" then
				table.insert(items, { filename = join(search_root, line) })
			end
		end
